    ...
    <div className="space-y-6 pb-12 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 sm:p-6 shadow-sm space-y-4">
          ...
          <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700/60 text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 sm:p-6 shadow-sm">
          ...
          <div className="grid grid-cols-7 gap-2 text-center text-xs font-mono">
            {sentiment.sentiment_trend.map((t, idx) => (
              <div key={idx} className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 flex flex-col justify-between h-32">
                <span className="text-slate-400 text-[11px]">{t.date}</span>
                <div className="my-2">
                  <span className={`text-base font-bold ${
                    t.sentiment_score > 0.15 ? 'text-emerald-500' : t.sentiment_score < -0.15 ? 'text-rose-500' : 'text-blue-500'
                  }`}>
                    {t.sentiment_score > 0 ? '+' : ''}{t.sentiment_score}
                  </span>
                </div>
                <span className={`text-[10px] px-1.5 py-0.5 rounded ${
                  t.sentiment_score > 0.15 ? 'bg-emerald-500/10 text-emerald-500' : t.sentiment_score < -0.15 ? 'bg-rose-500/10 text-rose-500' : 'bg-blue-500/10 text-blue-500'
                }`}>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 sm:p-6 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <div className="text-[11px] font-mono px-2.5 py-1 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-500">
        ...
        {(!sentiment.recent_articles || sentiment.recent_articles.length === 0) ? (
          ...
          <div className="space-y-3">
            {sentiment.recent_articles.map(art => (
              <div
                key={art.id}
                className="p-4 rounded-xl border border-slate-200 dark:border-slate-800 hover:border-indigo-500/30 dark:hover:border-indigo-500/30 bg-slate-50/50 dark:bg-slate-800/30 transition-all flex flex-col md:flex-row md:items-center justify-between gap-4"
              >
                <div className="space-y-1.5 flex-1">
                  ...
                  <a
                    ...
                    className="font-semibold text-sm text-slate-900 dark:text-white hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors block"
                  >
                ...
                <div className="flex items-center gap-3 self-start md:self-auto font-mono text-xs flex-shrink-0">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-3 py-1 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg border border-slate-200 dark:border-slate-700 transition-colors flex items-center gap-1 text-[11px]"
                  >
                    <span>Read</span>
